...
public class ProceduralTextureGeneration : MonoBehaviour {
	...
	private Texture2D _GenerateProceduralTexture () {
		Texture2D proceduralTexture = new Texture2D (_textureWidth, _textureWidth);
			...
			for (int h = 0; h < _textureWidth; h++) {
				...
				for (int i = 0; i < 3; i++) {
					for (int j = 0; j < 3; j++) {
						...
						Color color = _MixColor (_circleColor, new Color (pixel.r, pixel.g, pixel.b, 0.0f), Mathf.SmoothStep (0f, 1.0f, dist * edgeBlur));
